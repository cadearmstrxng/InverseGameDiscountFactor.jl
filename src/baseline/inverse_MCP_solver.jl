...
# function InverseMCPProblem(
#     game::CollisionAvoidanceGame,
#     horizon,
#     context_state_block_dimensions = 0;
#     observation_index = nothing,
#     prior_parmas_dim = 3,
#     params_processing_fn = nothing,
# )   
...
#     z, context_state, x0 = let
#         @variables(z[1:trajectory_size], context_state[1:sum(context_state_block_dimensions)], x0[1:state_dimension]) .|>
#         scalarize
#     end
#     x0 = BlockVector(x0, state_block_dimensions)
#     context_state =
...
#         G[τ_idx_set[ii]] = Symbolics.gradient(local_lagrangian, z[τ_idx_set[ii]])
#     end
#     private_inequality_idx_set = collect((trajectory_size + 1):(trajectory_size + length(private_duals)))

#     lb = vcat(fill(-Inf, trajectory_size), lb)
#     ub = vcat(fill(Inf, trajectory_size), ub)
#     # append private duals to z
#     z = vcat(z, private_duals)
#     G = vcat(G, private_constraints)
...
#         G = vcat(G, coupled_inequality_constraints)
...
#     outer_lagrangian = out_cost - outer_equality_duals' * G
#     f = Symbolics.gradient(outer_lagrangian, z)
#     z = vcat(z, outer_equality_duals)
#     f = vcat(f, G)
#     # lb = vcat(fill(-Inf, (length(z) - length(F))), fill(0.0, length(F)))
#     # # lb = fill(-Inf, length(z))
#     # ub = fill(Inf, length(z))
