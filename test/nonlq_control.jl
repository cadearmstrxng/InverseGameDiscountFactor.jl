...
using InverseGameDiscountFactor.ForwardOptimalControl: solve_optimal_control
using InverseGameDiscountFactor.InverseOptimalControl: solve_inverse_optimal_control
using InverseGameDiscountFactor.TrajectoryVisualization: visualize_trajectory
using InverseGameDiscountFactor.CostUtils: Symbol
...
function add_forward_objective!(opt_model, x, u; weights)
    ...
    @NLconstraint(
        opt_model,
        [t = 1:T],
        prox_cost[t] == -log(opt_model[:regularized_sq_dist][t])
    )
    ...
    @objective(
        opt_model,
        Min,
        sum(weights[k] * g̃[symbol(k)] for k in keys(weights))
    )
end
